...
<div class="col-lg-12 card">
    ...
    <a class="btn btn-info float-right px-4" href="{{ route('album.photo.index',$photo->album_id) }}">برگشت</a>
    </div>
    <!-- /.card-header -->
    <div class="card-body">
        <form class="needs-validation" action="{{ route('photo.update',$photo->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
          <div class="row g-3 my-3">
            <div class="col-8">
              <label for="txtname" class="form-label">عنوان</label>
              <input type="text" class="form-control" id="txtname" name="txtname" value="{{ $photo->title }}" >
            </div>
            <div class="col-4"></div>
          </div>
          ...
          <div class="row g-3 my-3">
            ...
            <div class="col-4">
                <span><img src="{{ asset('front/image/gallery/'.$photo->image) }}" alt="" width="80px" height="80px"></span>
            </div>
          </div>
          ...
          <div class="row g-3 my-3">
            <div class="col-8">
              ...
              <textarea id="txtdescription" class="form-control" name="txtdescription" cols="40" rows="5">{{ $photo->description }}</textarea>
            </div>
            <div class="col-4"></div>
          </div>
